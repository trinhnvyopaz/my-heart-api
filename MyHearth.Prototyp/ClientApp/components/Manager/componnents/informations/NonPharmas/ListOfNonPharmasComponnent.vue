                ...
                <template v-slot:body="{ items }">
                    <tbody>
                        <tr v-for="item in items" :key="item.id" v-on:click="openFarmak(item)">
                            ...
                            <td>{{ item.lengthOfHospitalization }}</td>
                        </tr>
                    </tbody>
                </template>
...
<script lang="ts">
    ...
    export default class ListOfNonPharmasComponnent extends Vue {
        ...
        openFarmak(item) {
            if (item)
                this.$router.push({ path: '/non-pharma/' + item.id });
            else
                this.$router.push({ path: '/non-pharma/' + 0 });
        }
